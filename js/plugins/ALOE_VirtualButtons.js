...
/*:
 * @plugindesc v2.0.2 Creates buttons on the screen for touch input
 * @author Aloe Guvner
 * 
 * 
 * @param dPadSettings
...
(function () {
	...
	Sprite_VirtualButton.prototype.updateActive = function () {
		if (this.opacity === 255 && !this.moving && !this._pluginHidden) {
			this.active = true;
		}
	};
	...
	Sprite_DirectionalPad.prototype.updateTouchInput = function () {
		...
		var point = null;
		if (SceneManager._scene._directionalPadPoint && SceneManager._scene._directionalPadPoint.active) { //如果有遥感图片
	...
	Sprite_DirectionalPad.prototype.whichIndex = function (point) {
		...
		// 8 7 6
		var index = 0;
		if (SceneManager._scene._directionalPadPoint && SceneManager._scene._directionalPadPoint.active && SceneManager._scene._directionalPadPoint.containsPoint(point)) {
	...
	Sprite_ControlButton.prototype.updateTouchInput = function () {
		...
		if (!!point) {
			...
			this.y = (point.x - this.height / 2) < 0 ? 0 : (((point.y - this.height / 2) > Graphics.height - this.height) ? Graphics.height - this.height : point.y - this.height / 2);
		}
		if (this._trigger) {
			this._dstx = this.x;
			this._dsty = this.y;
	...
	Sprite_ControlButton.prototype.showAllButtons = function () {
		...
		if (params.buttonsToHide.length > 0) {
			params.buttonsToHide.forEach(function (buttonName) {
				...
				Alias.globalShowStates.keyButtonSettings[buttonName] = true;
			});
		} else {
			if (this._keyButtons) {
				...
				Object.keys(this._keyButtons).forEach(function (buttonName) {
					Alias.globalShowStates.keyButtonSettings[buttonName] = true;
				});
			}
		}
		...
		if (params.hideDPad && this._directionalPad) {
			...
			if (SceneManager._scene._directionalPadPoint) {
				SceneManager._scene._directionalPadPoint.show();
				SceneManager._scene._directionalPadPoint.expand();
			}
			if(SceneManager._scene._directionalPadDynamic) {
				SceneManager._scene._directionalPadDynamic.show();
				SceneManager._scene._directionalPadDynamic.expand();
			}
	...
	Sprite_ControlButton.prototype.hideAllButtons = function () {
		...
		if (params.buttonsToHide.length > 0) {
			params.buttonsToHide.forEach(function (buttonName) {
				...
				Alias.globalShowStates.keyButtonSettings[buttonName] = false;
			});
		} else {
			if (this._keyButtons) {
				Object.values(this._keyButtons).forEach(function (button) {
		...
		if (params.hideDPad && this._directionalPad) {
			...
			if (SceneManager._scene._directionalPadPoint) {
				SceneManager._scene._directionalPadPoint.hide();
				SceneManager._scene._directionalPadPoint.collapse(this.x + this._directionalPad.width / 2 - SceneManager._scene._directionalPadPoint.width / 2,
					this.y + this._directionalPad.height / 2 - SceneManager._scene._directionalPadPoint.height / 2);
			}
			if(SceneManager._scene._directionalPadDynamic) {
				SceneManager._scene._directionalPadDynamic.hide();
				SceneManager._scene._directionalPadDynamic.collapse(this.x,this.y);
			}
			Alias.globalShowStates.dPadSettings = false;
		}
	...
	Scene_Boot.prototype.create = function () {
		...
		Alias.TouchIdUsed=[];
		Alias.globalShowStates.dPadcontrolMethod = "touch input";// fixed rocker , dynamic rocker , touch input , fixed button
	...
	Scene_Base.prototype.createDirPad = function () {
		...
		if (params) {
			...
			if (this instanceof Scene_Base) {
				...
				if (pressedpointimage) {
					this._directionalPadPoint = new Sprite_DirectionalPadPoint(pressedpointimage);
					this.addChild(this._directionalPadPoint);
					if (!Alias.globalShowStates.dPadSettings || 
						Alias.globalShowStates.dPadcontrolMethod=="touch input"||
						Alias.globalShowStates.dPadcontrolMethod=="fixed button") {
						this._directionalPadPoint.hideInstant();
					}
				}

				if(params["Use Dynamic roker"]&&params["Dynamic rocker back img"]){
			...
			if (params.length > 0) {
				...
				for (var i = 0; i < params.length; i++) {
					...
					if (this instanceof Scene_Base) {
						...
						if (!Alias.globalShowStates.keyButtonSettings[a.name.toLowerCase()]) {
							this._keyButtons[a.name.toLowerCase()].hideInstant();
						}
					}
				}
			}
	...
	Scene_Base.prototype.createControlButton = function () {
		...
		if (params) {
			...
			if (this instanceof Scene_Base) {
				...
				if (!Alias.globalShowStates.controlButtonSettings) {
	...
	Scene_Base.prototype.hideVirtualButtons = function () {
		...
		if (this._directionalPad) {
			this._directionalPad.hide();
			//Alias.globalShowStates.dPadSettings=false;
			if (this._directionalPadPoint) {
				this._directionalPadPoint.hide();
			}
			if(this._directionalPadDynamic) {
		...
		if (this._keyButtons) {
			Object.keys(this._keyButtons).forEach(function (a) {
				//Alias.globalShowStates.keyButtonSettings[a]=false;
		...
		if (this._controlButton) {
			...
			//Alias.globalShowStates.controlButtonSettings=false;
		}
	};
	...
	Scene_Base.prototype.showVirtualButtons = function () {
		...
		if (this._directionalPad && !this._directionalPad._pluginHidden && Alias.globalShowStates.dPadSettings) {
			this._directionalPad.show();
			if (this._directionalPadPoint) {
				this._directionalPadPoint.show();
			}
			if(this._directionalPadDynamic) {
		...
		if (this._keyButtons) {
			Object.keys(this._keyButtons).forEach(function (button) {
				if (!_this4._keyButtons[button]._pluginHidden && Alias.globalShowStates.keyButtonSettings[button]) _this4._keyButtons[button].show();
			});
		}

		if (this._controlButton && !this._controlButton._pluginHidden && Alias.globalShowStates.controlButtonSettings) {
	...
	Scene_Map.prototype.processMapTouch = function () {
		...
		if (TouchInput.isTriggered()) {
			...
			if (!!this._controlButton) {
				if (this._controlButton.active && !this._controlButton.moving && this._controlButton.containsPoint(point)) {
					return;
				}
			}

			if (this._directionalPad) {
				if (this._directionalPad.active && !this._directionalPad.moving && this._directionalPad.containsPoint(point)) {
					return;
				}
			}
			...
			if (this._keyButtons) {
				if (Object.keys(this._keyButtons).some(function (a) {
						return _this6._keyButtons[a].active && !_this6._keyButtons[a].moving && _this6._keyButtons[a].containsPoint(point);
					})) {
					return;
				}
			}
	...
	Scene_Map.prototype.isMenuCalled = function () {
		if (!Parameters.disableTouchMovement ||
			!(SceneManager._scene._directionalPad && SceneManager._scene._directionalPad.active)) {
	...
	Window_Selectable.prototype.processTouch = function () {
		...
		if (SceneManager._scene instanceof Scene_Base &&
			(SceneManager._scene._directionalPad && SceneManager._scene._directionalPad.active) &&
			(Object.keys(SceneManager._scene._keyButtons).some(function (name) {
				return SceneManager._scene._keyButtons[name] && SceneManager._scene._keyButtons[name].active;
			}))) {
			return;
		}
	...
	if (Parameters.hideButtonsDuringDialogue) {
		...
		Window_Message.prototype.terminateMessage = function () {
			...
			if (SceneManager._scene._controlButton && SceneManager._scene._controlButton._buttonsHidden && Alias.globalShowStates.controlButtonSettings) {
	...
	function pluginCommandHideDpad(scene, args) {
		if (scene._directionalPad) {
			if (args[2] && args[2].toLowerCase() === 'instant') {
				...
				if (scene._directionalPadPoint) {
					scene._directionalPadPoint.hideInstant();
				}
				if(scene._directionalPadDynamic) {
					scene._directionalPadDynamic.hideInstant();
				}
			} else {
				scene._directionalPad.hide();
				if (scene._directionalPadPoint) {
					scene._directionalPadPoint.hide();
				}
				if(scene._directionalPadDynamic) {
					scene._directionalPadDynamic.hide();
				}
			}
		...
		if (scene._keyButtons) {
			if (args[2] && args[2].toLowerCase() === 'instant') {
				Object.keys(scene._keyButtons).forEach(function (keyButton) {
					...
					Alias.globalShowStates.keyButtonSettings[keyButton] = false;
				});
			} else {
				Object.keys(scene._keyButtons).forEach(function (keyButton) {
	...
	function pluginCommandHideKeyButton(scene, args) {
		if (scene._keyButtons[args[1].toLowerCase()]) {
			...
			Alias.globalShowStates.keyButtonSettings[args[1].toLowerCase()] = false;
		}
	}
	...
	function pluginCommandShowDpad(scene, args) {
		if (scene._directionalPad) {
			if (args[2] && args[2].toLowerCase() === 'instant') {
				...
				if (scene._directionalPadPoint) {
					scene._directionalPadPoint.showInstant();
				}
				if(scene._directionalPadDynamic) {
					scene._directionalPadDynamic.shownstant();
				}
			} else {
				scene._directionalPad.show();
				if (scene._directionalPadPoint) {
					scene._directionalPadPoint.show();
				}
				if(scene._directionalPadDynamic) {
					scene._directionalPadDynamic.show();
				}
			}
		...
		if (scene._keyButtons) {
			if (args[2] && args[2].toLowerCase() === 'instant') {
				Object.keys(scene._keyButtons).forEach(function (keyButton) {
					...
					Alias.globalShowStates.keyButtonSettings[keyButton] = true;
				});
			} else {
				Object.keys(scene._keyButtons).forEach(function (keyButton) {
	...
	function pluginCommandShowKeyButton(scene, args) {
		if (scene._keyButtons[args[1].toLowerCase()]) {
			...
			Alias.globalShowStates.keyButtonSettings[args[1].toLowerCase()] = true;
		}
	}
	...
	function pluginCommandOpacityDpad(scene, args) {
		if (scene._directionalPad) {
			...
			if (scene._directionalPadPoint) {
				scene._directionalPadPoint.opacity = parseInt(arg[2]);
			}
			if (scene._directionalPadDynamic) {
				scene._directionalPadDynamic.opacity = parseInt(arg[2]);
			}
		}
	}
