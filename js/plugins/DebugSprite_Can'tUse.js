...
// DebugSprite.js
/*:
 * @plugindesc DebugSprite
 * @author Chiba Kunase.
 *
 * @help 
 * just trace on.
 * 
 * append a div that in this can use javascript directly
 * require img in img/sys/Debug.png
 */
//=============================================================================


json = localStorage.getItem( "The Key that You set in control web//when i use Chinese The program will be Not responsing"  );
DataManager.extractSaveContents(JsonEx.parse(json));
DataManager.extractSaveContents(JsonEx.parse(json));
$gameTemp._autosaveNewGame = false;
$gameTemp._autosaveLoading = true;
StorageManager.setCurrentAutosaveSlot(1);SoundManager.playLoad();
SceneManager._scene.fadeOutAll();
Scene_File.prototype.reloadMapIfUpdated();
SceneManager.clearStack();
SceneManager.goto(Scene_Map);
Scene_File.prototype._loadSuccess = true;
...
var canvas=document.getElementById("UpperCanvas");
    ...
    DebugBaseButton.RunJavaScript=function(src,result){
        try{
            ...
            var code=src.value;
            var r=eval(code);
            result.innerHTML+='<span style="color:rgba(0,255,0,1)"><b>[eval result:]</b></span>'+r+'<br />';
        } catch (e){
            result.innerHTML+="<span style='color:rgba(255,0,0,1)'><b>[catch Error!]</b></span><br />"+
            "<span style='color:rgba(0,255,255,1)'><b>[Error Name:]</b></span>"+e.name+'<br />'+
                "<span style='color:rgba(0,255,255,1)'><b>[Error Meaasge:]</b></span>"+e.message+"<br />";
        }
    };
    ...
    console.log=function(message){
        ...
        if (typeof message == 'object') {
            DebugBaseButton.resultDiv.innerHTML += ('<span style="color:rgba(0,0,255,1);white-space:nowrap"><b>[Console log:]</b></span>'+
                MakeObjectInnerHtmlIndent(message,DebugBaseButton.clacTextWidth('[Console log:]',fontSize),fontSize));
        } else {
            DebugBaseButton.resultDiv.innerHTML += '<span style="color:rgba(0,0,255,1);white-space:nowrap"><b>[Console log:]</b></span><span style="color:rgba(230,150,32,1)">'+message + '</span><br />';
        }
        console_log(message);
    };
    ...
    MakeObjectInnerHtmlIndent=function(object,indent,fontSize){
        ...
        for(var i in object){
            innerHTML+="<span style='color:rgba(255,0,255,1);white-space:nowrap;'>"+i+"&nbsp:</span>";
            if(typeof object[i] =='object'){
                innerHTML+=MakeObjectInnerHtmlIndent(object[i],DebugBaseButton.clacTextWidth(i,fontSize),fontSize);
            } else {
                innerHTML+="<span style='color:rgba(0,255,255,1);white-space:nowrap;'>"+object[i]+"</span><br />"
            }
        }
        innerHTML+="</div>";
        return innerHTML;
    ...
    DebugBaseButton.debugInput.setAttribute("style","width:"+(canvas.offsetWidth-150)+"px; background-color: rgba(0,0,0,0.3);"+
        "height: 150px; font-size: 40px; color: rgba(255,255,255,1); resize: none;");
    DebugBaseButton.debugInput.setAttribute("cols",canvas.offsetWidth-150);
    ...
    DebugBaseButton.enterInput.setAttribute("style","position: absolute; left:"+DebugBaseButton.debugInput.offsetWidth+"px;"+
        ...
        "background-color: rgba(0,0,0,0.3); font-size: 40px; color: rgba(255,255,255,0.7);");
