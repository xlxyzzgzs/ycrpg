...
Sprite_Minimap.prototype.drawMapForegroundGrid = function(bitmap, x, y)
{
    ...
    if (!passage.isPassableDir(x, y, 2))  // 下方向移動不可(不能向下移动)
    {
        dh -= 1;
    }
    if (!passage.isPassableDir(x, y, 4))  // 左方向移動不可(不能向左移动)
    {
        dx += 1;
        dw -= 1;
    }
    if (!passage.isPassableDir(x, y, 6))  // 右方向移動不可(不能向右移动)
    {
        dw -= 1;
    }
    if (!passage.isPassableDir(x, y, 8))  // 上方向移動不可(不能向上移动)
    {
        dy += 1;
        dh -= 1;
    }
 ...
 *(更新显示位置)
 *
 * @param {Point} begin      - ミニマップの表示開始座標(小地图显示开始坐标)
 * @param {Point} scrollDiff - 位置補正のためのマップスクロール量(地图滚动量以进行位置校正)
 */
