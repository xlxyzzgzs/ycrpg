...
DataManager.isDatabaseLoaded=function(){
	if(!DataManager_isDatabaseLoaded.call(this,arguments)) return false;
...
DataManager.processHelpNotetags=function(group){
	for (var n = 1; n < group.length; n++) {
		var obj = group[n];
		var notedata = obj.note.split(/[\r\n]+/);
	
		obj.beforeDescriptionEval="";
		obj.afterDescriptionEval="";
		var evalMode = 'none';
		
		for (var i = 0; i < notedata.length; i++) {
		  var line = notedata[i];
		  if (line.match(/<BEFORE DESCRIPTION>/i)) {
			evalMode = 'before';
		  } else if (line.match(/<\/BEFORE DESCRIPTION>/i)) {
			evalMode = 'none';
		  } else if (evalMode === 'before') {
			obj.beforeDescriptionEval+=line+'\n';
		  } else if (line.match(/<AFTER DESCRIPTION>/i)) {
			evalMode = 'after';
		  } else if (line.match(/<\/AFTER DESCRIPTION>/i)) {
			evalMode = 'none';
		  } else if (evalMode === 'after') {
			obj.afterDescriptionEval+=line+'\n';
		  }
	    }
	}
};
...
Window_FloatHelp.prototype.setItem = function(item) {
	var text="";
	if(item){
		text=this.processEval(item.beforeDescriptionEval,item);
		text+=item.description;
		text+=this.processEval(item.afterDescriptionEval,item);
	}
    this.setText(text);
};

Window_FloatHelp.prototype.processEval=function(code,item){
	if(code=="") {return ;}
	var mainItem = item;
    var weapon = item;
    var armor = item;
