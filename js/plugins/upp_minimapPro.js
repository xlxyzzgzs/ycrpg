...
( function() {
	...
	*/
	...
	Minimap_Display.prototype.refreshDraw = function(){
		...
		//绘制图像的位置（估计）
		this.left=this.x;
		this.right=this.x+this.bitmap.width*this.scale.x;
		this.top=this.y;
		this.bottom=this.y+this.bitmap.height*this.scale.y;
		//
	}
	
	Minimap_Display.prototype.update = function(){
		$gameSystem.mmData.plugins.mainUpdate.call(this);
		upp_miniMap_visible.call(this);
	}
	...
	Minimap_Visuals.prototype.initialize = function(){
		...
		//绘制图像的位置（估计）
		this.left=this.x;
		this.right=this.x+this.bitmap.width;
		this.top=this.y;
		this.bottom=this.y+this.bitmap.height;
		//
		this.drawBorder();
	...
	Minimap_LocName.prototype.initialize = function(){
		...
		//绘制图像的位置（估计）
		this.left= $gameSystem.mmData.nameOffset.x;
		this.right= $gameSystem.mmData.nameOffset.x+$dataMap.width*$gameSystem.mmData.scale;
		this.top=$gameSystem.mmData.nameOffset.y;
		this.bottom=$gameSystem.mmData.nameOffset.y+this.lineHeight();
		//
	}
	...
	Minimap_LocName.prototype.refresh = function(){
		...
		this.contents.drawText(($gameMap.locationDisplayName?$gameMap.locationDisplayName:$dataMapInfos[$gameMap.mapId()]["name"]), 0, 0, this.contents.width, this.lineHeight(), 'center');
	}
	...
	*/
	...
	window.Scene_mmSettings=Scene_mmSettings;
	...
	*/
	...
	Scene_Map.prototype.start = function(){
		...
		if(ConfigManager.ShowMiniMap==true)
		{
			$mmForceCreate = true;
		}
		if($gameSystem.mmData.visualSettings!="Off")
		{
			if (mmItemCheck>0) {
				...
				if(($gameMap.locationDisplayName?$gameMap.locationDisplayName:$dataMapInfos[$gameMap.mapId()]["name"]))
				{
	...
	/* 为了适应YEP 而改变*/
	mmDataManager=DataManager.extractSaveContents;
	DataManager.extractSaveContents = function(contents) {
		tempPlugins=$gameSystem.mmData.plugins;
		mmDataManager.call(this, contents);
		$gameSystem.mmData.plugins=tempPlugins;
	};
	window.upp_miniMap_visible=function(){
		if (upp_miniMap_show.call(this)) {
			if (this.alpha<0.95){
				this.alpha += 0.05;
			}
		}	 
			...
			if ($gameMessage.isBusy()) {
				...
				if (this.alpha > 0.2) {	
					this.alpha -= 0.05;
					if (this.alapha < 0.2) {this.alpha=0.2};
				};
			};
		};
	};
	window.upp_miniMap_show=function(){
		if ($gameMessage.isBusy()) {return false};
		if ($gamePlayer.screenX() < this.left-25) {return true};
		if ($gamePlayer.screenX() > this.right+25) {return true};
		if ($gamePlayer.screenY() > this.bottom+70) {return true};
		if ($gamePlayer.screenY() < this.top+25) {return true};
		if (this.alpha < 0.2) {return true};
		return false;
	}
